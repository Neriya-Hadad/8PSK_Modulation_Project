...
% Generate 8PSK constellation points
constellation_points = exp(1i * (0:M-1) * 2*pi/M); % Symbol mapping on the unit circle
...
for snr_idx = 1:length(SNR_dBs)
    ...
    noise_var = 1 / (2 * SNR);               % AWGN noise variance
...
% Plot simulated vs. theoretical BER vs. SNR (dB)
figure;
